...
export default function UserDetails({toggleUserEditDetails,isApiMessage,collectEditdata,message,originalData}) {
    //redux code
     let userId =localStorage.getItem("token");
     const [userDetails, setuserDetails] = useState({
        userId:userId,
        yourName:originalData.username,
        universityName:originalData.universityName,
        collegeName:originalData.collegeName,
        permanentAddress:originalData.permanentAddress,
        presentAddress:originalData.presentAddress,
    ...
    return (
        <div className="bg-indigo-100 fixed w-5/6 xl:w-2/5   top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 rounded-md transition-all ease-in-out duration-700">
            <div className="flex justify-between">
               ...
               <div className="m-1">
                    <svg onClick={toggleUserEditDetails}  xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 mx-2  mt-1 cursor-pointer text-red-500 hover:text-red-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
